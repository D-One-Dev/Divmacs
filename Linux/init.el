...
;; ---------------------------------------------------------
;; Linux keyboard behavior
;; ---------------------------------------------------------
;;
;; Alt  = Meta
;; Super = Super
;;
;; These are normally already the defaults on Linux, so there
;; is no need for the macOS-specific mac-command-modifier /
;; mac-option-modifier variables.
...
;; ---------------------------------------------------------
;;
;; Keep the same Command-based shortcuts from macOS, but use
;; Super on Linux.
;;
...
(use-package treemacs
  :ensure t
  :bind
  ;; Auto-detect the current project (project.el, falling back to the
  ;; current directory) instead of prompting for a root.
  ("C-t" . treemacs-add-and-display-current-project)
  :config
  (setq treemacs-width 60
        treemacs-position 'left
        treemacs-is-never-other-window t)
  ;; Force treemacs into a left side window regardless of any other
  ;; display-buffer rule that may route it elsewhere (e.g. bottom).
